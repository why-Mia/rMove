...
class SettingsController extends Controller
{
    ...
    public function check_username(\App\Http\Requests\CheckUsernameRequest $request){
        ...
        $users_request = new ApiRequest($url);
        $users_request->setData($data);
        $users_request->sendPostRequest();
        $user_data = $users_request->getResponseData();
        if($user_data !== false && property_exists($user_data, 'data') && array_key_exists(0,$user_data->data)){
            ...
            if(property_exists($user_data,'id')){
                ...
                return json_encode(['user_exists' => false, 'error_in_response' => false, 'data' => $user_data]);
            }
        }
        else{
            return  json_encode(['error_in_response' => true, 'data' => $user_data]);
        }
    }
    ...
    public function verify_account(\App\Http\Requests\VerifyAccountRequest $request){
        ...
        $users_request->sendPostRequest();
        $user_data = $users_request->getResponseData();
        if($user_data !== false && property_exists($user_data->data[0], 'id') && property_exists($user_data->data[0], 'name') ){    
            $user_data = $user_data->data[0];     
              
            //description get request
            $description_request = new ApiRequest('https://users.roblox.com/v1/users/'.$user_data->id);
            $description_request->sendGetRequest();
            $description_data = $description_request->getResponseData();

            //avatar img get request
            $avatar_request = new ApiRequest('https://thumbnails.roblox.com/v1/users/avatar?userIds='.$user_data->id.'&size=250x250&format=Png&isCircular=false');
            $avatar_request->sendGetRequest();
            $avatar_data = $avatar_request->getResponseData();
            if($avatar_data !== false && $description_data !== false && property_exists($avatar_data->data[0], 'imageUrl') && property_exists($description_data, 'description')){
                //Check if user description contains verification code
                if(str_contains($description_data->description, auth()->user()->roblox_verification_code)){ 
                    $user = auth()->user();
                    ...
                    //TODO: this might cause errors when deleting the account since the table of times relies on it
                    ...
                    if(RobloxAccount::where('id',$user_data->id)->exists()){
                        $roblox_account = RobloxAccount::where('id',$user_data->id)->first();
                    }
                    ...
                    $roblox_account->id = $user_data->id;
                    $roblox_account->username = $user_data->name;
                    $roblox_account->displayname = $user_data->displayName;
                    $roblox_account->avatar_image_url = $avatar_data->data[0]->imageUrl;
                    if($user->robloxAccounts()->where('id',$user_data->id)->exists()){
                        $user->robloxAccounts()->where('id',$user_data->id)->first()->update($roblox_account);
                    }
                    else{
                        $user->robloxAccounts()->save($roblox_account);
                    }
                    $newtimestxt = TimesController::refresh($user_data->id);
                    \Session::flash('message','Roblox account successfully linked <br>'.$newtimestxt);
    ...
    public function unlink_account(\App\Http\Requests\UnlinkAccountRequest $request){
        if(auth()->user()->robloxAccounts()->where('id',$request['roblox_id'])->exists()){
            $roblox_account = auth()->user()->robloxAccounts()->where('id',$request['roblox_id'])->first();
            $roblox_account->user_id = null;
            auth()->user()->robloxAccounts()->where('id',$request['roblox_id'])->update(['user_id' => null, 'is_primary_account' => null]);
    ...
    private function refresh_times($user){
